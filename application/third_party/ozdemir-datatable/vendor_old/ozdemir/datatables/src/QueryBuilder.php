...
class QueryBuilder
{
     ...
     *
     */
    public function setFullQuery($order_key = null): void
    {
        $this->full = clone $this->filtered;
        $this->full->set($this->filtered . $this->orderBy($order_key) . $this->limit());
    }
    ...
    protected function filterGlobal(Query $query): string
    {
        ...
        foreach (explode(' ', $searchinput) as $word) {
            ...
            foreach ($columns as $column) {
                $look[] = $column->name.' ILIKE '.$this->db->escape('%'.$word.'%', $query);
            }
     ...
     * @return string
     */
    protected function orderBy($order_key = null): string
    {
        // if ($order_key != null) {
        //     return ' ORDER BY ' . $order_key;
        // }
